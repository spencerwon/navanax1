...
| `com.navanax.dashboard` | `python3 -m navanax.cli dashboard --no-browser --port 8765` | Same |
...
- **`ThrottleInterval: 10`** — launchd will not restart a job more often than every 10 seconds. Without it, a job that fails instantly is respawned in a hot loop.
