...
Plus one **opt-in** job that `autostart-install.command` deliberately does *not* install: `com.navanax.keepawake` (§8.6). And one **flag-gated** job that exists only when `stream.redundant.enabled` is true: `com.navanax.recorder-b`, the second stream connection (§8.13). While the flag is false — which is the default — that job is not written, not loaded, and not on the machine.
...
| `2` | Configuration: empty watchlist; no usable `OPENSEA_API_KEY` in `.env`; or (with `--redundant`) the redundant flag is off, `OPENSEA_API_KEY_2` is missing, or it is the same key as A's (§8.13). |
...
### 8.13 The second recorder (PR-10), and why there is usually no such job

*Added 2026-09-11.*

`stream.redundant.enabled` in `config/base.yaml` is **false by default**, and while it is false there is **no second job on the machine at all** — not a disabled one, not a stopped one. `tools/launchd.py` reads that flag through the same two functions the recorder reads it with (`navanax.cli._config` → `navanax.redundancy.settings`, environment overlay included), and `autostart-install.command` asks it for the label list rather than carrying its own copy. Turn the flag on and `com.navanax.recorder-b` joins the three jobs the installer writes; turn it off and it leaves again.

The B job is supervised exactly like A — `KeepAlive`, `RunAtLoad`, `ThrottleInterval: 10`, `ExitTimeOut: 30` — because the argument for supervising A applies unchanged to B: an hour it did not record cannot be bought back. What differs is three things. It runs `ingest --redundant --supervised`. It writes to `data/logs/recorder-b.log`, so one recorder's restart loop can be told from the other's in the logs rather than by guesswork. And it takes **its own** `.ingest.lock`, under `data/landing-b/`, so A's lock is free and both processes genuinely run — two writers on one landing root silently lose manifest records, which is the failure `_single_instance` exists to prevent and which two roots avoid by construction.

Three things will make the B job refuse and exit 2, and all three are configuration rather than faults: the flag is off; `OPENSEA_API_KEY_2` is missing from `.env`; or it is the *same* key as `OPENSEA_API_KEY`. The last one is refused on purpose. Free instant keys expire after seven days (REQ-D-06), so two processes on one key fail in the same minute on a known schedule — which is precisely the moment the second connection was meant to be covering the first. Create the two keys on different days. Under `KeepAlive` a refusal loops every 10 s and says the same sentence into `recorder-b.log` each time, which is the intended way to notice.

**Turning the flag back off removes the job.** `autostart-install.command` computes the label list from `tools/launchd.py labels`, compares it with what `launchctl list` actually has loaded, and **boots out and deletes** any job of ours this configuration no longer wants — printing each one. Without that, a `com.navanax.recorder-b` left loaded after the flag went off would be restarted by `KeepAlive` every ten seconds forever, refusing each time with exit 2 into a log nobody is reading. A `com.navanax.*` job this version cannot *name* is reported and **left alone**: it is likelier to belong to a newer version of this project than to be rubbish, and booting out something we cannot name is how an upgrade breaks a downgrade.
