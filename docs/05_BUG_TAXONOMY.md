...
`docs/logs/bugs.yaml` is the source of truth; `tools/buglog.py` generates `docs/logs/BUGS.xlsx` — a sortable spreadsheet with severity, priority, class, root cause, resolution, code index, GitHub links, regression test and monitor gap, plus a severity-hierarchy sheet with live counts. `tools/buglog.py --check` runs in CI and fails the build if a bug marked fixed names a regression test that does not exist, or if any `BUG-` id referenced anywhere in the repo is absent from the ledger. (This paragraph previously specified a `BUGS_INDEX.md` regenerated by `docs-steward`. That file was never built and nothing generated it — a documented capability that did not exist, which is the shape this taxonomy exists to catch. The spreadsheet does the job it described.)
